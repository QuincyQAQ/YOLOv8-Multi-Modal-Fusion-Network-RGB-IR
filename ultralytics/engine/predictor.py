        ...
        with self._lock:  # for thread-safe inference
            ...
            for batch in self.dataset:
                ...
                for i in range(n):
                    ...
                    p = Path(p)
                    w = list(self.results[i].speed.keys())[1][0]
                    name = str(p.name[:-4])+str(w)+(p.name[-4:])
                    if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                        s += self.write_results(i, self.results, (p, im[:,:3], im0[...,-3:]))
                    if self.args.save or self.args.save_txt:
                    ...
                    if self.list is not None:
                        if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                            self.write_results(i, self.list, (p, im[:, -3:], im0[..., :3]))
                        if self.args.save and self.plotted_img is not None:
                            self.save_preds(vid_cap, i, str(self.save_dir / name))
